<!-- App Header Wrapper-->
      ...
      <?php
if (empty($conn) || empty($doctor_id)) return;

$today = date('Y-m-d');

// 1. Pending regimens for doctor review
$pending_regimens = [];
$r = $conn->query("
    SELECT tr.id, p.patient_code, tr.created_at,
    ...
    GROUP BY tr.id ORDER BY tr.created_at ASC LIMIT 5
");
if ($r) $pending_regimens = $r->fetch_all(MYSQLI_ASSOC);

// 2. Unreviewed lab results
$pending_labs = [];
$r = $conn->query("
    SELECT lr.id, p.patient_code, lr.test_type, lr.result_date
    FROM lab_results lr
    JOIN patients p ON lr.patient_id = p.id
    WHERE lr.is_final = 0
    ORDER BY lr.created_at DESC LIMIT 3
");
if ($r) $pending_labs = $r->fetch_all(MYSQLI_ASSOC);

// 3. Low adherence count (<85% in 30d)
$low_adh_count = 0;
$r = $conn->query("
    SELECT COUNT(*) FROM (
        SELECT patient_id, ROUND((SUM(status IN ('taken','late'))/COUNT(*))*100,1) AS adh
        FROM adherence_logs
        WHERE dose_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
        GROUP BY patient_id HAVING adh < 85
    ) t
");
if ($r) $low_adh_count = (int)$r->fetch_column();

// 4. Severe / life-threatening AEs
$severe_aes = [];
$r = $conn->query("
    SELECT ae.id, p.patient_code, ae.event_type, ae.severity, d.drug_code
    FROM adverse_events ae
    JOIN patients p ON ae.patient_id = p.id
    LEFT JOIN drugs d ON ae.drug_id = d.id
    WHERE ae.severity IN ('severe','life_threatening')
      AND ae.resolution_date IS NULL
    ORDER BY ae.onset_date DESC LIMIT 5
");
if ($r) $severe_aes = $r->fetch_all(MYSQLI_ASSOC);

$severe_ae_count = count($severe_aes);

// 5. Recent decisions (approved/rejected regimens)
$decisions = [];
$r = $conn->query("
    SELECT tr.id, tr.status, tr.reviewed_at, tr.review_notes,
           p.patient_code,
           GROUP_CONCAT(CONCAT(d.drug_code) SEPARATOR ' + ') AS drug_summary
    FROM treatment_regimens tr
    JOIN patients p ON tr.patient_id = p.id
    LEFT JOIN regimen_drugs rd ON tr.id = rd.regimen_id AND rd.is_active = 1
    LEFT JOIN drugs d ON rd.drug_id = d.id
    WHERE tr.reviewed_by = {$doctor_id}
      AND tr.status IN ('active','rejected')
      AND tr.reviewed_at IS NOT NULL
    GROUP BY tr.id
    ORDER BY tr.reviewed_at DESC LIMIT 5
");
if ($r) $decisions = $r->fetch_all(MYSQLI_ASSOC);
?>
          ...
          <div x-data="{activeTab:'tabHome'}" class="relative flex h-full w-full transform-gpu flex-col bg-white transition-transform duration-200 dark:bg-navy-750" x-show="$store.global.isRightSidebarExpanded" x-transition:enter="ease-out" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="ease-in" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full">
            <div class="flex items-center justify-between py-2 px-4">
              <p x-show="activeTab === 'tabHome'" class="flex shrink-0 items-center space-x-1.5">
                ...
                <span class="text-xs">Review Queue</span>
              </p>
            ...
            <div x-show="activeTab === 'tabHome'" x-transition:enter="transition-all duration-500 easy-in-out" x-transition:enter-start="opacity-0 [transform:translate3d(0,1rem,0)]" x-transition:enter-end="opacity-100 [transform:translate3d(0,0,0)]" class="is-scrollbar-hidden overflow-y-auto overscroll-contain pt-1">
              <div class="mt-3 px-3">
                <h2 class="text-xs+ font-medium tracking-wide text-slate-700 line-clamp-1 dark:text-navy-100">Pending Reviews</h2>
                <div class="mt-3 space-y-2">
                  <?php if (empty($pending_regimens)): ?>
    <p class="text-sm text-slate-400 py-3 text-center">No pending reviews</p>
<?php else: foreach ($pending_regimens as $pr):
    $diff = time() - strtotime($pr['created_at']);
    $ago  = $diff < 3600 ? round($diff/60).'m ago' : round($diff/3600).'h ago';
?>
                  <div class="rounded-lg border border-warning/30 bg-warning/5 p-3 dark:bg-warning/10">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="badge rounded-full bg-warning/10 px-2 py-0.5 text-xs text-warning">Pending</span>
                    </div>
                    <p class="mt-1 text-xs text-slate-400"><?= htmlspecialchars($pr['drug_summary'] ?? 'No drugs assigned') ?></p>
                    <p class="mt-0.5 text-xs text-slate-300">Submitted <?= $ago ?></p>
                    <div class="mt-2 flex space-x-2">
                      <a href="regimen_reviews.php?id=<?= $pr['id'] ?>&action=approve" class="btn flex-1 h-7 rounded-lg bg-success text-xs text-white hover:bg-success/90">Approve</a>
                      <a href="regimen_reviews.php?id=<?= $pr['id'] ?>" class="btn flex-1 h-7 rounded-lg bg-slate-150 text-xs text-slate-700 hover:bg-slate-200 dark:bg-navy-500 dark:text-navy-100">Review</a>
                    </div>
                  </div>
                  <?php endforeach; endif; ?>
                  <!-- <div class="rounded-lg border border-warning/30 bg-warning/5 p-3 dark:bg-warning/10">
                    <div class="flex items-center justify-between">
                      <p class="text-sm font-medium text-slate-700 dark:text-navy-100">MDR-2025-0039</p>
                      <span class="badge rounded-full bg-warning/10 px-2 py-0.5 text-xs text-warning">Pending</span>
                    </div>
                    <p class="mt-1 text-xs text-slate-400">MFX 400mg, BDQ 400mg, LZD 600mg</p>
                    <div class="mt-2 flex space-x-2">
                      <a href="regimen_reviews.php" class="btn flex-1 h-7 rounded-lg bg-success text-xs text-white hover:bg-success/90">Approve</a>
                      <a href="regimen_reviews.php" class="btn flex-1 h-7 rounded-lg bg-slate-150 text-xs text-slate-700 hover:bg-slate-200 dark:bg-navy-500 dark:text-navy-100">Review</a>
                    </div>
                  </div> -->
                </div>
              </div>
              ...
              <div class="mt-4 px-3">
                <h2 class="text-xs+ font-medium tracking-wide text-slate-700 line-clamp-1 dark:text-navy-100">Preliminary Lab Results</h2>
                <div class="mt-3 space-y-2">
                  <!-- PENDING LAB RESULTS -->
<?php if (empty($pending_labs)): ?>
    <p class="text-sm text-slate-400 py-3 text-center">No pending labs</p>
<?php else: foreach ($pending_labs as $lb):
    $diff = time() - strtotime($lb['result_date']);
    $ago  = $diff < 3600 ? round($diff/60).'m ago' : round($diff/3600).'h ago';
?>
                  <div class="flex items-center justify-between rounded-lg bg-info/5 border border-info/20 p-3 dark:bg-info/10">
                    <div>
                      <p class="text-sm font-medium text-slate-700 dark:text-navy-100"><?= htmlspecialchars($lb['patient_code']) ?></p>
                      <p class="text-xs text-slate-400"><?= htmlspecialchars($lb['test_type']) ?> &mdash; <?= $ago ?></p>
                    </div>
                    <a href="lab_review.php?id=<?= $lb['id'] ?>" class="btn size-7 rounded-full bg-info/20 p-0 text-info hover:bg-info/30">
              ...
              <div class="mt-4 grid grid-cols-2 gap-3 px-3">
                <div class="rounded-lg bg-slate-100 p-3 dark:bg-navy-600">
                  <div class="flex justify-between">
                    <p class="text-xl font-semibold text-slate-700 dark:text-navy-100"><?= $low_adh_count ?></p>
                ...
                <div class="rounded-lg bg-slate-100 p-3 dark:bg-navy-600">
                  <div class="flex justify-between">
                    <p class="text-xl font-semibold text-slate-700 dark:text-navy-100"><?= $severe_ae_count ?></p>
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-5 text-warning" fill="none" viewbox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"></path></svg>
                  </div>
                  <p class="mt-1 text-xs+">Severe AEs</p>
                </div>
              </div>
            ...
            <div x-show="activeTab === 'tabProjects'" x-transition:enter="transition-all duration-500 easy-in-out" x-transition:enter-start="opacity-0 [transform:translate3d(0,1rem,0)]" x-transition:enter-end="opacity-100 [transform:translate3d(0,0,0)]" class="is-scrollbar-hidden overflow-y-auto overscroll-contain px-3 pt-1">
              <div class="mt-3">
                <h2 class="text-xs+ font-medium tracking-wide text-slate-700 line-clamp-1 dark:text-navy-100">Severe / Life-threatening AEs</h2>
                <div class="mt-3 space-y-2">
                  <!-- SEVERE AEs -->
                <?php if (empty($severe_aes)): ?>
                    <p class="text-sm text-slate-400 py-3 text-center">No critical alerts</p>
                <?php else: foreach ($severe_aes as $ae): ?>
                  <div class="rounded-lg border border-error/30 bg-error/5 p-3 dark:bg-error/10">
                    <div class="flex items-center justify-between">
                      <p class="text-sm font-medium text-slate-700 dark:text-navy-100"><?= htmlspecialchars($ae['patient_code']) ?></p>
                      <span class="badge rounded-full bg-error px-2 py-0.5 text-xs text-white"><?= ucfirst(str_replace('_',' ',$ae['severity'])) ?></span>
                    </div>
                    <p class="mt-1 text-xs text-slate-400"><?= htmlspecialchars($ae['event_type']) ?><?= !empty($ae['drug_code']) ? ' — '.$ae['drug_code'] : '' ?></p>
                    <a href="adverse_events.php" class="mt-2 inline-flex items-center text-xs text-error hover:text-error/80">Manage <svg class="ml-1 size-3" fill="none" viewbox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"></path></svg></a>
                  </div>
                  <?php endforeach; endif; ?>
                  <!-- <div class="rounded-lg border border-error/30 bg-error/5 p-3 dark:bg-error/10">
                    <div class="flex items-center justify-between">
                      <p class="text-sm font-medium text-slate-700 dark:text-navy-100">MDR-2025-0018</p>
                      <span class="badge rounded-full bg-error px-2 py-0.5 text-xs text-white">Life-threat</span>
                    </div>
                    <p class="mt-1 text-xs text-slate-400">Hepatotoxicity - MFX</p>
                    <a href="adverse_events.php?id=<?= $ae['id'] ?>"class="mt-2 inline-flex items-center text-xs text-error hover:text-error/80">Manage <svg class="ml-1 size-3" fill="none" viewbox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"></path></svg></a>
                  </div> -->
                </div>
              </div>
              <div class="h-18"></div>
...
<?php if (empty($decisions)): ?>
    <p class="mt-6 text-center text-sm text-slate-400">No decisions yet</p>
<?php else: foreach ($decisions as $dec):
    $diff    = time() - strtotime($dec['reviewed_at']);
    $ago     = $diff < 3600 ? round($diff/60).'m ago' : ($diff < 86400 ? round($diff/3600).'h ago' : round($diff/86400).'d ago');
    $approved = $dec['status'] === 'active';
?>
                <li class="timeline-item">
                  <div class="timeline-item-point rounded-full border border-current bg-white <?= $approved ? 'text-success' : 'text-error' ?> dark:bg-navy-700"><i class="fa <?= $approved ? 'fa-check' : 'fa-times' ?> text-tiny"></i></div>
                  <div class="timeline-item-content flex-1 pl-4">
                    ...
                    Regimen <?= $approved ? 'Approved' : 'Rejected' ?>
                </p>
                      <span class="text-xs text-slate-400"><?= $ago ?></span>
                    </div>
                    <p class="py-1"><?= htmlspecialchars($dec['patient_code']) ?><?= !empty($dec['drug_summary']) ? ' — '.$dec['drug_summary'] : '' ?></p>
                  </div>
                </li>
                <!-- <li class="timeline-item">
                  ...
                  <div class="timeline-item-content flex-1 pl-4">
                    <div class="flex flex-col justify-between pb-2 sm:flex-row sm:pb-0">
                      <p class="pb-2 font-medium leading-none text-slate-600 dark:text-navy-100 sm:pb-0">Regimen Rejected</p>
                      <span class="text-xs text-slate-400">2 hours ago</span>
                    </div>
                    <p class="py-1">MDR-2025-0038 — Insufficient drugs for resistance profile</p>
                  </div>
                </li>
                <li class="timeline-item">
                  <div class="timeline-item-point rounded-full border border-current bg-white text-info dark:bg-navy-700"><i class="fa fa-flask text-tiny"></i></div>
                  <div class="timeline-item-content flex-1 pl-4">
                    <div class="flex flex-col justify-between pb-2 sm:flex-row sm:pb-0">
                      <p class="pb-2 font-medium leading-none text-slate-600 dark:text-navy-100 sm:pb-0">Lab Approved</p>
                      <span class="text-xs text-slate-400">3 hours ago</span>
                    </div>
                    <p class="py-1">MDR-2025-0033 GeneXpert marked as final</p>
                  </div>
                </li> -->
              </ol>
              <?php endforeach; endif; ?>
