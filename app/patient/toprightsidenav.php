...
if (!empty($regimen)) {
    $stmt = $conn->prepare("
        SELECT
            d.drug_name,
            d.drug_name       AS drug_code,
            d.default_dose_mg,
            d.default_dose_mg AS dose_mg,
            d.unit,
            ms.dose_time,
        ...
        JOIN drugs d ON d.id = ms.drug_id
        LEFT JOIN adherence_logs al
            ON  al.schedule_id = ms.id
            AND al.patient_id  = ?
            AND al.dose_date   = ?
        WHERE ms.regimen_id = ?
          AND ms.is_active  = 1
    ...
    ");
    $stmt->bind_param("isi", $patient_id, $today, $regimen['id']);  // ← was missing
    $stmt->execute();                                                // ← was missing
    $today_meds = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    $stmt->close();
}

// ─────────────────────────────────────────────────────────────────────────────
// 2. WEEKLY ADHERENCE — single GROUP BY query, no loop
// ─────────────────────────────────────────────────────────────────────────────
...
$stmt = $conn->prepare("
    ...
    FROM adherence_logs
    WHERE patient_id = ?
      AND dose_date BETWEEN ? AND ?
    GROUP BY dose_date
    ORDER BY dose_date ASC
");
$stmt->bind_param("iss", $patient_id, $seven_days_ago, $today);
$stmt->execute();
$weekly_rows = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt->close();

// Index by date for O(1) gap-fill
$weekly_by_date = [];
foreach ($weekly_rows as $row) {
    $weekly_by_date[$row['dose_date']] = $row;
}
...
for ($i = 6; $i >= 0; $i--) {
    ...
    $r     = $weekly_by_date[$day] ?? ['total' => 0, 'taken' => 0, 'late' => 0, 'missed' => 0];
    $taken  = (int)$r['taken'];
    $late   = (int)$r['late'];
    $total  = (int)$r['total'];
    $weekly[] = [
        ...
        'late'   => $late,
        'missed' => (int)$r['missed'],
        'pct'    => $total > 0 ? round(($taken + $late) / $total * 100) : 0,
    ];
}
...
if (!empty($regimen['start_date'])) {
    $weeks_in     = max(0, (int)((time() - strtotime($regimen['start_date'])) / (7 * 24 * 3600)));
    $pct_progress = min(100, round(($weeks_in / $total_weeks) * 100));
}
...
if (!empty($regimen)) {
    $stmt = $conn->prepare("
        SELECT COUNT(*)
        FROM regimen_drugs
        WHERE regimen_id = ? AND is_active = 1
    ");
    $stmt->bind_param("i", $regimen['id']);
    $stmt->execute();
    $drug_count = (int)$stmt->get_result()->fetch_column();
    $stmt->close();
}

// ─────────────────────────────────────────────────────────────────────────────
// 5. AGGREGATE STATS — single query
//    overall adherence + total taken all-time + today taken
//    NULLIF avoids division-by-zero when no logs exist yet
// ─────────────────────────────────────────────────────────────────────────────
$stmt = $conn->prepare("
    SELECT
        SUM(status IN ('taken','late'))                              AS total_taken,
        SUM(status IN ('taken','late') AND dose_date = ?)           AS today_taken,
        ROUND(
            SUM(status IN ('taken','late')) / NULLIF(COUNT(*), 0) * 100
        , 1)                                                         AS overall_adh
    FROM adherence_logs
    WHERE patient_id = ?
");
...
$today_taken = (int)($stats['today_taken']  ?? 0);
$overall_adh = (float)($stats['overall_adh'] ?? 0.0);

// ─────────────────────────────────────────────────────────────────────────────
// 6. RECENT ACTIVITY — last 5 logs
//    logged_at doesn't exist → use created_at
//    drug_name comes via schedule_id → medication_schedule → drugs
// ─────────────────────────────────────────────────────────────────────────────
$stmt = $conn->prepare("
    SELECT
        al.status,
...
// HELPERS
// ─────────────────────────────────────────────────────────────────────────────
function med_style(string $status): array {
    return match ($status) {
        'taken'  => ['bg' => 'bg-success/10 dark:bg-success/15', 'icon_bg' => 'bg-success/20',                'icon' => 'text-success',  'label_cls' => 'text-success',  'label' => 'Taken'],
        'late'   => ['bg' => 'bg-warning/10 dark:bg-warning/15', 'icon_bg' => 'bg-warning/20',                'icon' => 'text-warning',  'label_cls' => 'text-warning',  'label' => 'Late'],
        'missed' => ['bg' => 'bg-error/10 dark:bg-error/15',     'icon_bg' => 'bg-error/20',                  'icon' => 'text-error',    'label_cls' => 'text-error',    'label' => 'Missed'],
        default  => ['bg' => 'bg-slate-100 dark:bg-navy-600',    'icon_bg' => 'bg-slate-200 dark:bg-navy-500','icon' => 'text-slate-400','label_cls' => 'text-slate-400','label' => 'Pending'],
    };
}

function adherence_badge(float $pct): string {
    if ($pct >= 85) return 'bg-success/10 text-success';
    if ($pct >= 70) return 'bg-warning/10 text-warning';
    return 'bg-error/10 text-error';
}

function adherence_label(float $pct): string {
    if ($pct >= 85) return 'Good';
    if ($pct >= 70) return 'Fair';
    return 'Low';
}

function weekly_bar_color(array $w): string {
    if ($w['total'] === 0) return 'bg-slate-200 dark:bg-navy-500';
    if ($w['missed'] > 0)  return 'bg-error';
    if ($w['late']   > 0)  return 'bg-warning';
    return 'bg-success';
}

function log_color(string $status): string {
    return match ($status) {
        'taken'  => 'success',
        'late'   => 'warning',
        'missed' => 'error',
...
function log_icon(string $status): string {
    return match ($status) {
        'taken'  => 'fa-check',
        'late'   => 'fa-clock',
        default  => 'fa-times',
        ...
        >
            ...
            <div class="flex items-center justify-between py-2 px-4">
                <p x-show="activeTab === 'tabHome'" class="flex shrink-0 items-center space-x-1.5">
                    ...
                    <span class="text-xs">My Treatment</span>
                </p>
            ...
            >
                ...
                <div class="mt-3 px-3">
                    ...
                    <div class="mt-3 space-y-2">
                        <?php if (empty($today_meds)): ?>
                            <p class="py-4 text-center text-sm text-slate-400 dark:text-navy-300">
                                No medications scheduled
                            </p>
                        <?php else: ?>
                            <?php foreach ($today_meds as $m):
                                $status = $m['log_status'] ?? 'pending';
                                $s      = med_style($status);
                                $label  = $s['label'];
                                if ($status === 'late' && !empty($m['actual_time_taken'])) {
                                    $label = 'Late ' . substr($m['actual_time_taken'], 0, 5);
                                }
                            ?>
                            <div class="flex items-center justify-between rounded-lg <?= $s['bg'] ?> p-3">
                                <div class="flex items-center space-x-3">
                                    <div class="flex size-8 items-center justify-center rounded-full <?= $s['icon_bg'] ?>">
                                        <?php if ($status === 'taken'): ?>
                                            <svg xmlns="http://www.w3.org/2000/svg" class="size-4 <?= $s['icon'] ?>" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-slate-400">
                                            <?= (int)$m['default_dose_mg'] ?><?= htmlspecialchars($m['unit'] ?? 'mg') ?>
                                            &mdash; <?= substr($m['dose_time'], 0, 5) ?>
                                        </p>
                                    </div>
                                </div>
                                <span class="text-xs font-medium <?= $s['label_cls'] ?>">
                ...
                <div class="mt-4 px-3">
                    ...
                    <div class="mt-3 rounded-lg bg-slate-100 p-3 dark:bg-navy-600">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="badge rounded-full <?= adherence_badge($overall_adh) ?> px-2 py-0.5 text-xs font-medium">
                                <?= adherence_label($overall_adh) ?>
                            </span>
                        </div>
                        <div class="mt-3 flex items-end space-x-1" style="height:2.5rem;">
                            <?php foreach ($weekly as $w):
                                $bar_color = weekly_bar_color($w);
                                $bar_h     = $w['total'] === 0 ? '10%' : $w['pct'] . '%';
                            ?>
                            <div class="flex flex-1 flex-col items-center justify-end h-full">
                                <div
                                    class="w-full rounded-sm <?= $bar_color ?> transition-all duration-300"
                                    style="height:<?= $bar_h ?>"
                                ></div>
                            </div>
                            <?php endforeach; ?>
                        </div>
                        <div class="mt-2 flex justify-between text-tiny text-slate-400 dark:text-navy-300">
                            <?php foreach ($weekly as $w): ?>
                                <span><?= $w['day'] ?></span>
                            <?php endforeach; ?>
                        </div>
                ...
                <div class="mt-4 px-3">
                    ...
                    <div class="mt-3 grid grid-cols-2 gap-3">
                        <div class="rounded-lg bg-slate-150 px-2.5 py-2 dark:bg-navy-600">
                            <p class="text-lg font-medium text-slate-700 dark:text-navy-100">Week <?= $weeks_in ?></p>
                            <p class="text-tiny+ uppercase text-slate-400">of <?= $total_weeks ?> weeks</p>
                            <div class="progress mt-2 h-1.5 bg-primary/15 dark:bg-accent/25">
                                <div class="rounded-full bg-primary dark:bg-accent" style="width:<?= $pct_progress ?>%"></div>
                            </div>
                        </div>
                        <div class="rounded-lg bg-slate-150 px-2.5 py-2 dark:bg-navy-600">
                            <p class="text-lg font-medium text-slate-700 dark:text-navy-100"><?= $drug_count ?></p>
                            <p class="text-tiny+ uppercase text-slate-400">Drugs Active</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4 px-3">
                    ...
                    <div class="mt-3 rounded-lg border border-slate-150 p-3 dark:border-navy-600">
                        <div class="flex items-center space-x-3">
                            <div class="flex size-10 items-center justify-center rounded-full bg-error/10">
                                <svg xmlns="http://www.w3.org/2000/svg" class="size-5 text-error" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z"/>
                                </svg>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-slate-700 dark:text-navy-100">Report Side Effects</p>
                                <a href="report_side_effect.php" class="text-xs text-primary hover:text-primary-focus dark:text-accent-light dark:hover:text-accent">
            ...
            >
                <div class="mt-3">
                    ...
                    <div class="mt-3 space-y-2">
                        ...
                        <a href="medications.php" class="flex items-center space-x-3 rounded-lg bg-slate-100 p-3 hover:bg-slate-150 dark:bg-navy-600 dark:hover:bg-navy-500">
                            ...
                            <div>
                                <p class="text-sm font-medium text-slate-700 dark:text-navy-100">My Medications</p>
                                <p class="text-xs text-slate-400">View all prescribed drugs</p>
                            </div>
                        </a>

                        <a href="results.php" class="flex items-center space-x-3 rounded-lg bg-slate-100 p-3 hover:bg-slate-150 dark:bg-navy-600 dark:hover:bg-navy-500">
                            ...
                            <div>
                                <p class="text-sm font-medium text-slate-700 dark:text-navy-100">Lab Results</p>
                                <p class="text-xs text-slate-400">View test results</p>
                            </div>
                        </a>

                        <a href="adherence.php" class="flex items-center space-x-3 rounded-lg bg-slate-100 p-3 hover:bg-slate-150 dark:bg-navy-600 dark:hover:bg-navy-500">
                            ...
                            <div>
                                <p class="text-sm font-medium text-slate-700 dark:text-navy-100">Adherence Report</p>
                                <p class="text-xs text-slate-400">View adherence history</p>
                            </div>
                        </a>

                        <a href="appointments.php" class="flex items-center space-x-3 rounded-lg bg-slate-100 p-3 hover:bg-slate-150 dark:bg-navy-600 dark:hover:bg-navy-500">
                            <div class="flex size-10 items-center justify-center rounded-lg bg-primary/10 dark:bg-accent/10">
                                <svg xmlns="http://www.w3.org/2000/svg" class="size-5 text-primary dark:text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5"/>
                                </svg>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-slate-700 dark:text-navy-100">Appointments</p>
                                <p class="text-xs text-slate-400">View upcoming visits</p>
                            </div>
                        </a>
            ...
            >
                ...
                <div class="mx-3 mt-3 rounded-lg bg-slate-100 py-3 px-4 dark:bg-navy-600">
                    <div class="flex items-center justify-between">
                        <p class="text-xs text-slate-400">Doses Taken</p>
                        <p class="text-lg font-medium text-slate-700 dark:text-navy-100"><?= $total_taken ?></p>
                    </div>
                    <div class="mt-1 flex items-center justify-between">
                        <p class="text-xs text-slate-400">Since Start</p>
                        <span class="text-xs font-medium text-success">+<?= $today_taken ?> today</span>
                    </div>
                </div>

                <!-- Timeline -->
                <?php if (empty($recent_logs)): ?>
                    <p class="mt-6 text-center text-sm text-slate-400 dark:text-navy-300">No activity yet</p>
                <?php else: ?>
                <ol class="timeline line-space mt-5 px-4 [--size:1.5rem]">
                    <?php foreach ($recent_logs as $l):
                        $l_color = log_color($l['status']);
                        $l_icon  = log_icon($l['status']);
                        $l_time  = date('h:i A', strtotime($l['created_at']));
                    ?>
                    <li class="timeline-item">
                        <div class="timeline-item-point rounded-full border border-current bg-white text-<?= $l_color ?> dark:bg-navy-700">
                            <i class="fa <?= $l_icon ?> text-tiny"></i>
                        </div>
                        <div class="timeline-item-content flex-1 pl-4">
                            <div class="flex flex-col justify-between pb-2 sm:flex-row sm:pb-0">
                                <p class="pb-2 font-medium leading-none text-slate-600 dark:text-navy-100 sm:pb-0">
                                    <?= htmlspecialchars($l['drug_name']) ?>
                                    <?= ucfirst($l['status']) ?>
                                </p>
                                <span class="text-xs text-slate-400"><?= $l_time ?></span>
                            </div>
                            <?php if ($l['status'] === 'late' && !empty($l['actual_time_taken'])): ?>
                                <p class="py-1 text-sm text-slate-400">
                                    Taken at <?= htmlspecialchars(substr($l['actual_time_taken'], 0, 5)) ?>
                                </p>
                            <?php elseif ($l['status'] === 'missed' && !empty($l['notes'])): ?>
                                <p class="py-1 text-sm text-slate-400">
                                    <?= htmlspecialchars($l['notes']) ?>
                                </p>
                            <?php else: ?>
                                <p class="py-1 text-sm text-slate-400">
                                    Dose recorded as <?= $l['status'] ?>
                                </p>
                            <?php endif; ?>
                        </div>
                    </li>
